...
export class RabbitMQService implements OnModuleInit, OnModuleDestroy {
  ...
  private readonly queueName = 'q.gamification.campaign-events';
      ...
      this.channel.consume(
        ...
        async (msg) => {
          ...
          try {
            ...
            this.logger.log(`Event alındı [${routingKey}]: ${content.event_id}`);
            ...
            this.channel?.nack(msg, false, false); // DLQ'ya düşer
          }
        },
        { noAck: false },
      );
    } catch (error) {
      this.logger.warn(`RabbitMQ bağlantısı kurulamadı (Gamification event dinleyici beklemede): ${error.message}`);
    }
  }
  ...
  async publishEvent(eventType: string, payload: any) {
    ...
    if (published) {
      this.logger.log(`Event [${eventType}] başarıyla yayınlandı.`);
    }
